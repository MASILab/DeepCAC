...
def generate_bash_script_for_batch_process_incomplete_only():
    n_batch = 4
    script_dir = '/nfs/masi/xuk9/Projects/DeepCAC/NLST_full_run_script_incomplete_v1'
    if not os.path.exists(script_dir): os.makedirs(script_dir)

    chunk_dir = '/nfs/masi/xuk9/Projects/DeepCAC/NLST_full'
    chunk_list = os.listdir(chunk_dir)

    # Get the chunk list with incomplete result
    incomplete_chunk_list = []
    for chunk in chunk_list:
        result_csv_path = os.path.join(chunk_dir, chunk, 'step4_cac_score', 'cac_score_results.csv')
        if not os.path.exists(result_csv_path):
            incomplete_chunk_list.append(chunk)

    chunk_batch_list = np.array_split(incomplete_chunk_list, n_batch)
...
if __name__ == '__main__':
    ...
    generate_bash_script_for_batch_process_incomplete_only()
